...
4. **Fix privately** — develop the fix without referencing the vulnerability in public commits/PRs; backport to every supported line and rebuild affected container images.
5. **Release** — cut and vote the fixed releases (source + images) through the normal ASF release process so the patched versions are available before disclosure. *(manual)*
